...
export { PermanentLink } from './sequelize/PermanentLink'
...
export const db = new Sequelize(process.env.PG_CONNECT_STRING || '', {
  ...
  modelMatch: (filename: string, member: string) => filename[0].toUpperCase() === filename[0] && member.startsWith(filename)
})
